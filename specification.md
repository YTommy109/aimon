...
- **外部ライブラリ**:
  - Excelファイル (`.xlsx`) の読み込みには`openpyxl`ライブラリを使用します。
- **環境変数管理**: direnv
...
- **各行のレイアウト**: 以下の要素を左から順にカラムで表示します。
    ...
    6. **実行ボタン**: 各行の「詳細」ボタンの横に「実行」ボタンを配置します。実行可能なプロジェクト（ステータスがPendingのもの）のみ表示し、実行開始後（Running/Processing/Completed/Failed）は非表示とします。
- **詳細表示**:
...
- **実行エンジン**:
  - 本来、AIツールは**Azure FunctionsのAPIエンドポイント**として提供され、本アプリケーションはそれを呼び出すことを想定しています。
  - 現在はAzure Functions環境が未整備であるため、暫定的な措置として**Google Gemini API (`gemini-1.5-flash`モデル) を直接呼び出し**ています。
...
- **AIツール一覧表示**:
  ...
  - 「状態」は`disabled_at`が`null`なら「有効」、そうでなければ「無効」と表示します。
- **AIツール登録**:
  - 「新規ツール登録」ボタンを配置します。
  - ボタンを押すとモーダルダイアログが開き、以下の項目を入力させます。
    ...
    - ツール名 (`name_ja`)
    - 説明 (`description`)
  - 「登録」ボタンで`ai_tools.json`に追記します。その際、`created_at`と`updated_at`に現在日時を設定し、`disabled_at`は`null`として保存します。
- **AIツール編集**:
  - 「編集」ボタンを押すと、登録時と同様のモーダルが開き、既存の情報を編集できます。
  - 編集を保存する際に`updated_at`を現在日時に更新します。
- **AIツール状態変更 (有効化/無効化)**:
  - ボタンは現在の状態に応じて「無効化」または「有効化」と表示されます。
  - **無効化する場合**: `disabled_at`と`updated_at`に現在日時を設定します。
  - **有効化する場合**: `disabled_at`を`null`に設定し、`updated_at`を現在日時に更新します。
  - **物理的な削除は行いません。**
...
- **基本方針**: テストの再現性と保守性を確保するため、テストデータは原則としてテストコード内で動的に生成します。
- **例外 (複雑なファイル形式)**:
  - セルの結合や画像の挿入を含む、いわゆる「神エクセル」のような複雑な構造を持つファイルは、動的な生成が困難です。
  - これらのファイルは、あらかじめテストデータとしてリポジトリ内の適切な場所（例: `tests/fixtures/`）に配置し、テスト実行時にそれを読み込んで使用します。
