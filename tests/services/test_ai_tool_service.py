...
class TestAIToolService:
    ...
    def test_存在しないIDでAIツールを取得するとValueErrorが発生する(
        ...
        mock_repository.find_by_id.side_effect = ResourceNotFoundError('AIツール', tool_id)
    ...
    def test_AIツールを更新できる(
        ...
        # Assert
        assert result is True
        mock_repository.find_by_id.assert_called_once_with(sample_ai_tool.id)
        mock_repository.save.assert_called_once()
    ...
    def test_存在しないAIツールの更新は失敗する(
        ...
        # Act
        result = service.update_ai_tool(
            tool_id, '更新されたツール', '更新された説明', 'https://api.example.com/updated'
        )
